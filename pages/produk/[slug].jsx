...
export default function ProductDetail({ product, related }) {
  ...
  const savings = hasWA && hasMarket && product.wa_price < product.price ? product.price - product.wa_price : 0
  ...
  function handleOrderWA() {
    ...
    if (hasWA) msg += `Harga WA: *${fmt(product.wa_price)}*\n`
    else if (hasMarket) msg += `Harga: ${fmt(product.price)}\n`
  ...
  return (
    <>
      <SEO
        title={product.name}
        description={product.description || `Beli ${product.name} ${product.brand ? 'dari ' + product.brand : ''} di Anugerah Sports Malang. 100% original, harga terbaik. Pesan via WhatsApp!`}
      ...
      <div className="bg-white min-h-screen">
        ...
        <div className="bg-sand border-b border-gray-100 py-3">
          <div className="max-w-7xl mx-auto px-6 flex items-center gap-2 text-xs text-gray-400 flex-wrap">
            ...
            {product.cat && <>
              <span>›</span>
              <Link href={`/catalog?cat=${encodeURIComponent(product.cat)}`} className="hover:text-red transition-colors">{product.cat}</Link>
            </>}
            <span>›</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                {product.cat && (
                  <Link href={`/catalog?cat=${encodeURIComponent(product.cat)}`}
              ...
              <div className="flex flex-col gap-2">
                {hasWA && (
                  <div className="bg-green-50 border-2 border-green-300 rounded-xl px-5 py-4">
                    ...
                    {savings > 0 && (
                      <p className="text-sm text-green-700 mt-1">Hemat {fmt(savings)} ({savePct}%) dibanding marketplace</p>
                    )}
                  </div>
                )}
                {hasMarket && (
                  <div className="flex items-center gap-3 bg-sand rounded-xl px-4 py-3 flex-wrap">
                    ...
                    {hasSale && !hasWA && <span className="text-sm text-gray-400 line-through">{fmt(product.price)}</span>}
              ...
              <div className="flex flex-col gap-2">
                ...
                {(product.toko_url || product.shopee_url) && (
              ...
              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Bagikan Produk</p>
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      const url = typeof window !== 'undefined' ? window.location.href : ''
                      const msg = `Cek produk ini di Anugerah Sports: *${product.name}* — ${url}`
                      window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, '_blank')
                    }}
                    className="flex items-center gap-2 bg-green-500 text-white text-xs font-bold px-3 py-2 rounded-lg hover:bg-green-600 transition-all">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-sand border border-gray-200 text-xs font-bold px-3 py-2 rounded-lg hover:bg-gray-100 transition-all text-gray-600">
                    📋 Salin Link
                  </button>
                </div>
              </div>
...
export async function getServerSideProps({ params }) {
  const { slug } = params
  const { data: products } = await supabase.from('products').select('*').eq('slug', slug).limit(1)
  const product = products?.[0] || null
  let related = []
  if (product) {
    const { data } = await supabase.from('products').select('*').eq('cat', product.cat).neq('id', product.id).limit(5)
    related = data || []
  }
